...
class SecurityConfig {
    ...
    SecurityConfig(@Value("${app.security.jwt-secret}") String secret) {
        this.key = new SecretKeySpec(secret.getBytes(StandardCharsets.UTF_8), "HmacSHA256");
    }

    @Bean
    SecurityFilterChain filterChain(HttpSecurity http, JwtTenantFilter tenantFilter) throws Exception {
        return http
