...
| `event` | `pricingTiers` (2–6) | Always read with the event, never alone, tiny, and only mutable before publish. |
...
`event.pricingTiers` is the one that could honestly go either way. It is embedded here because it
is small and always read with the event. Against: RLS treated it as a separate thing, and an
unpriced tier is what blocks publishing, so it is read *about* rather than only read *with*. If
tiers ever became independently queryable — "every event with a tier under 50,000 ₫" — this would
be the first decision to revisit.
